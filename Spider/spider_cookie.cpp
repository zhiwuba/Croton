...
int Spider_Cookie::login_weibo(const char* account, const char* password)
{
	...
	std::string rsakv;
	std::string pubkey;
	int ret=sina_preloginstatus(en_account,servertime, nonce, rsakv, pubkey, cookie);
	if ( ret!=0 )
	{
	...
	do 
	{
		...
		if ( ret!=0 )
		{
			LLOG(L_ERROR,"LoginWeibo-->Sina_Dologin Failed!!!");
			break;
		}

		ret=weibo_ajaxlogin(weibo_url, cookie);
		if ( ret<=0 )
		{
			nret=ret;
			break;
		...
		else if ( ret==1 )
		{
			LLOG(L_WARN,"");
			need_code=1;
			ret=weibo_codes(cookie,verification);
			if ( ret!=0 )
			{
				LLOG(L_ERROR,"Weibo_Codes  error..");
				break;
			}
		}
	} while (need_code);
	...
	{
		LLOG(L_ERROR,"Failed  to login weibo.");
		delete cookie;
	}
										...
										std::string& nonce, std::string& rsakv,std::string& pubkey, Cookie* cookie)
{
	...
	request_line+="&rsakt=mod&checkpin=1&client=ssologin.js(v1.4.11) HTTP/1.1";
	...
	{
		LLOG2(L_ERROR,"Weibo_PreloadStatus recvbody error.body content: \n%s\n", body.c_str());
		return -1;
	}
								...
								std::string& nonce, std::string&rsakv, std::string& weibo_url, std::string& verification, Cookie* cookie)
{
	...
	sprintf(post_data,
		"entry=weibo&gateway=1&from=&savestate=7&useticket=1&pagerefer=&%svsnf=1&su=%s&service=miniblog&servertime=%s&nonce=%s&pwencode=rsa2&rsakv=%s&sp=%s&encoding=UTF-8&prelt=80&url=%s&returntype=META",
		verification.c_str() , en_acount.c_str(), servertime.c_str(), nonce.c_str(), rsakv.c_str(), en_password.c_str(), from_url.c_str());
	
	Http_Request_Package    request_package;
	Http_Response_Package  response_package; 
	request_package.set_request_line("POST /sso/login.php?client=ssologin.js(v1.4.11) HTTP/1.1");
	request_package.set_field("Accept","*/*");
	request_package.set_field("User-Agent","Mozilla/5.0");
	request_package.set_field("Host",  "login.sina.com.cn");
	request_package.set_field("Content-Length",strlen(post_data));
	...
	if ( ret!=200&&ret!=302 )
	{
		LLOG2(L_ERROR , "Weibo_Dologin surface_page error. ");
		return -1;
	}
	else
	{
	...
	{
		LLOG2(L_ERROR,"Failed to login weibo. recv location error.");
		return -1;
	}
	weibo_url=recv_data.substr(start_pos+18, end_pos-start_pos-18);
	return 0;
}

int Spider_Cookie::weibo_ajaxlogin(std::string url, Cookie* cookie)
{
	std::string host, uri, line;
	int port, ret;
	Http_Request_Package    request_package;
	...
	std::string recv_data;
	recv_data.assign(body, ret);
	int start_pos=recv_data.find("feedBackUrlCallBack(");
	int end_pos=recv_data.find(")", start_pos);
	if ( start_pos>=end_pos )
	{
		LLOG(L_ERROR,"Weibo_AjaxLogin recv content is error.");
		return -1;
	}

	std::string login_result;
	login_result=recv_data.substr(start_pos+20, end_pos-start_pos-20);
	Json::Value json_result;
	Json::Reader reader;
	if ( reader.parse(login_result, json_result) )
	{
		if ( json_result["result"].asBool() )
		{
			Json::Value userinfo=json_result["userinfo"];
			LLOG(L_TRACE,"Sucessed to login %s weibo.", userinfo["displayname"].asCString() );
		}
		else
		{
			std::string reason=unicode_to_ansi( json_result["reason"].asCString() );
			LLOG(L_WARN, "Failed to login weibo, reason: %s", reason.c_str());
			if ( json_result["errno"].asString().compare("4049")==0 )
			{	//需要输入验证码
				return 1;
			}
			return -1;
...
int Spider_Cookie::weibo_codes(Cookie* cookie, std::string& verification)
{//验证码
	//"http://login.sina.com.cn/cgi/pin.php?r=28407888&s=0&p=xd-7d8ec036c2e71bff740cb8c5553e960a402f";
	Http_Request_Package    request_package;
	Http_Response_Package  response_package; 
	
	request_package.set_request_line("GET /cgi/pin.php?r=28407888&s=0&p=xd-7d8ec036c2e71bff740cb8c5553e960a402f HTTP/1.1");
	request_package.set_field("Accept","*/*");
	request_package.set_field("User-Agent","Mozilla/5.0");
	request_package.set_field("Host",  "login.sina.com.cn");
	request_package.set_field("Cookie",cookie->to_string() );
	...
	std::string code;
	std::cin>>code;
	verification="pic=xd-7d8ec036c2e71bff740cb8c5553e960a402f&door="+code+"&";
	return 0;
}
